...
void main() {
  ...
  float interaction = texture2D(interactionFieldTex, vec2(u, v)).r * 2.0 - 1.0;
  float macro = mix(n, interaction, interactionFieldBlend);

  vec3 surfaceColor;

  if (surfType == SURF_TYPE_CONTINENTAL) {
    // -- Continental: balanced land/ocean --
    float landMask = smoothstep(-0.05, 0.1, macro);
    ...
    float h = smoothstep(0.1, 0.5, macro);
    vec3 land = mix(lowland, highland, h);
    float depth = smoothstep(-0.4, -0.05, macro);
    ...
    float depth = smoothstep(-0.4, -0.15, macro);
    vec3 waterC = mix(swampWater, murkyShallow, depth);
    float h = smoothstep(0.05, 0.4, macro);
    ...
    float ridge = smoothstep(-0.15, 0.45, macro);
    ...
    surfaceColor = mix(surfaceColor, pale, smoothstep(0.35, 0.7, macro));
    ...
    float highland = smoothstep(0.12, 0.55, macro);
    ...
    float ridge = smoothstep(-0.05, 0.45, macro);
    ...
    surfaceColor = mix(surfaceColor, snow, smoothstep(0.25, 0.65, macro));
    ...
    float ash = smoothstep(-0.25, 0.35, macro);
    ...
    float landMask = smoothstep(-0.18, 0.02, macro);
    ...
    vec3 waterC = mix(deepWater, shallow, smoothstep(-0.4, -0.08, macro));
    vec3 forest = mix(lowForest, highForest, smoothstep(0.0, 0.4, macro));
    forest = mix(forest, rock, smoothstep(0.35, 0.65, macro) * 0.2);
    ...
    ridge = pow(ridge, 2.5);
    float elevation = smoothstep(0.0, 1.0, macro * 0.5 + ridge * 0.5);
